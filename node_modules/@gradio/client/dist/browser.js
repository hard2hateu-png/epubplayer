...
async function be() {
	...
	if (e && (n.Authorization = `Bearer ${e}`), t) try {
		...
		if (typeof window < "u" && window.gradio_api_info) r = window.gradio_api_info;
			...
			let i = me(t.root, this.api_prefix, ee);
...
async function ke(e, t) {
	if (!(e.config?.root || e.config?.root_url)) throw Error(D);
	await Ae(e, t);
}
...
function Et(e, t, n) {
	...
	else if (n.type === "status" && n.original_msg === "process_starts") kt(i, n.time);
	else if (n.type === "status" && (n.stage === "generating" || n.stage === "streaming")) i.streamed = !0;
	else if (n.type === "status" && n.stage === "complete") i.status = "completed", At(i, n), a = !0;
	else if (n.type === "status" && n.stage === "error") i.status = "failed", i.error = typeof n.message == "string" ? n.message : JSON.stringify(n.message || "Unknown error"), At(i, n), a = !0;
	else return;
	if (a) Q.has(t) && (i.outputs = Q.get(t)), Q.delete(t), Ot.delete(t);
	else if (i.streamed) {
		if (Date.now() - (Ot.get(t) ?? 0) < Dt) return;
		Ot.set(t, Date.now()), Q.delete(t);
	}
	mt(e, r);
}
var Dt = 500, Ot = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map();
function kt(e, t) {
...
function Vt(e, t = {}, n, r, i, a) {
	try {
		let { token: o } = this.options, { fetch: s, app_reference: c, config: l, session_hash: f, api_info: p, api_map: m, stream_status: ee, pending_stream_messages: h, pending_diff_streams: te, event_callbacks: g, unclosed_events: ne, post_data: _, options: y, api_prefix: b } = this, x = a || { "x-gradio-user": "api" }, S = this;
		if (!p) throw Error(ce);
		if (!l) throw Error("Could not resolve app config");
		let { fn_index: C, endpoint_info: w, dependency: T } = Ut(p, e, m, l), oe = ye(t, w), E = l.protocol ?? "ws";
		if (E === "ws") throw Error(le);
		let se = typeof T.api_name == "string" ? `/${T.api_name}` : e, ue = typeof T.api_name == "string" ? `/${T.api_name}` : `Function ${C}`, D = (e) => {
		...
		}, de = T.api_visibility === "public", fe = l.run_history !== !1 && this.options.record_history !== !1, O = {
			app_id: l.app_id,
			username: l.username
		}, k = !fe || !de ? null : Lt({
			...O,
			...
			input_components: T.inputs.map(D),
			output_components: T.outputs.map(D)
		}), A, j = "", M = typeof e == "number" ? "/predict" : e, N, P = null, F = !1, I = {}, L = typeof window < "u" && typeof document < "u" ? new URLSearchParams(window.location.search).toString() : "", pe = y?.events?.reduce((e, t) => (e[t] = !0, e), {}) || {};
		function R(e) {
			zt(O, k, e), (i || pe[e.type]) && U(e);
		}
		async function me() {
			let e = {}, t = {};
			e = { event_id: P }, t = {
		...
		let _e = this.handle_blob(l.root, oe, w).then(async (e) => {
			...
			if (Rt(O, k, t || []), N = {
				...
				fn_index: C,
				trigger_id: r,
				...y.oauth_token && w?.oauth_token ? { oauth_token: y.oauth_token } : {}
			}, Te(C, l)) R({
				type: "status",
				endpoint: M,
				stage: "pending",
				...
				if (t == 200) R({
					...
					endpoint: M,
					fn_index: C,
					data: G(i, T, l.components, "output", y.with_null_state),
					time: /* @__PURE__ */ new Date(),
					event_data: n,
					trigger_id: r
				...
				A.onmessage = async function(e) {
					let { type: t, status: i, data: a } = ve(JSON.parse(e.data), I[C]);
					if (t === "update" && i && !F) R({
						type: "status",
						endpoint: M,
						fn_index: C,
					...
					a && (R({
						type: "data",
						time: /* @__PURE__ */ new Date(),
						data: G(a.data, T, l.components, "output", y.with_null_state),
						endpoint: M,
						fn_index: C,
						event_data: n,
						trigger_id: r
				...
				return (typeof window < "u" && typeof document < "u" && window.parent != window && t && window.supports_zerogpu_headers ? Ee("zerogpu-headers", t) : Promise.resolve(null)).then((e) => {
					...
					if (e.event_id && (j = e.event_id), t === 503) R({
						...
						let t = async function(e) {
							try {
								let { type: t, status: n, data: r, original_msg: i } = ve(e, I[C]);
								...
								if (t === "update" && n && !F) R({
									...
									"sse_v2.1",
									"sse_v3"
								].includes(E) && tt(te, P, r));
								r && (R({
									type: "data",
									time: /* @__PURE__ */ new Date(),
									data: G(r.data, T, l.components, "output", y.with_null_state),
									endpoint: M,
									fn_index: C
								...
								console.error("Unexpected client exception", e), R({
									...
									"sse_v2.1",
									"sse_v3"
								].includes(E) && (et(ee, S.abort_controller), ee.open = !1, V());
							}
						};
...
var $ = class {
	...
	constructor(e, t = { events: ["data"] }) {
		this.app_reference = e, this.deep_link = t.query_params?.deep_link || null, t.events ||= ["data"], O(t), this.options = t, this.current_payload = {}, t.cookies && (this.cookies = t.cookies), this.view_api = be.bind(this), this.upload_files = z.bind(this), this.handle_blob = Oe.bind(this), this.post_data = Me.bind(this), this.submit = Vt.bind(this), this.predict = Ne.bind(this), this.open_stream = $e.bind(this), this.resolve_config = j.bind(this), this.resolve_cookies = N.bind(this), this.upload = H.bind(this), this.fetch = this.fetch.bind(this), this.handle_space_success = this.handle_space_success.bind(this), this.stream = this.stream.bind(this);
	}
	async init() {
	...
	async refresh() {
		...
		if (!e) throw Error(y);
		this.config = e, this.api_prefix = e.api_prefix || "", this.api_map = k(e.dependencies || []);
		try {
			this.api_info = await this.view_api();
		} catch (e) {
	...
	async config_success(e) {
		if (this.config = e, this.api_prefix = e.api_prefix || "", e.run_history === !1 && Mt({
			app_id: e.app_id,
			username: e.username
...
async function Wt(e, t = { events: ["data"] }) {
...
//#endregion
export { $ as Client, U as FileData, oe as MISSING_CREDENTIALS_MSG, It as apply_run_history_replay, Mt as clear_run_history, Wt as client, Ft as consume_run_history_replay, Nt as delete_run_history, Gt as duplicate, De as handle_file, Bt as on_run_history_change, Ne as predict, xe as prepare_files, jt as read_run_history, lt as run_history_url, Pt as stage_run_history_replay, Vt as submit, o as t, H as upload, z as upload_files };
