...
# -----------------------------------------------------------------------------
# TTS manager: stale async initialization must never survive an engine/session
# change. The old code reset isInitialized only, leaving isInitializing and the
# old promise alive indefinitely.
# -----------------------------------------------------------------------------
path = "src/services/tts/ttsManager.ts"
replace(
    path,
...
# -----------------------------------------------------------------------------
# Voicebox service: invalidate old session work and bound network waits. A dead
# tunnel should produce a useful error, not an eternal Loading TTS engine state.
# -----------------------------------------------------------------------------
path = "src/services/tts/voiceboxRemoteService.ts"
replace(
    path,
    "  private initPromise: Promise<void> | null = null\n",
    "  private initPromise: Promise<void> | null = null\n  private initGeneration = 0\n",
...
"""
new_request = """    const timeoutMs = path === '/transcribe' || path.includes('/samples') ? 90_000 : 45_000
...
"""
replace(path, old_request, new_request)
replace(
    path,
...
# -----------------------------------------------------------------------------
# Playback: a remote Voicebox session must not initialize while simply opening
# a book/settings page. Initialize explicitly from Play. Also calculate the new
# engine's voice from the new engine, not the old one.
# -----------------------------------------------------------------------------
path = "src/features/player/PlaybackController.ts"
replace(
    path,
...
# -----------------------------------------------------------------------------
# Settings pairing: pairing a fresh Kaggle tunnel invalidates any previous
# Voicebox manager state immediately.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Setup sheet: connection test/activation should be quick and should not perform
# all first-use model/reference preparation or force a page reload. Play owns the
# expensive initialization so the UI stays predictable.
# -----------------------------------------------------------------------------
path = "src/features/settings/VoiceboxRemoteSetupSheet.tsx"
replace(
    path,
...
print("Voicebox playback-state patch applied")
